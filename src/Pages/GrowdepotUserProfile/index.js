...
const Gowdepaotuser = () => {
  ...
  const [growDepot, setGrowdepaot] = useState([]);
  ...
  const GetGrowdepaots = async (GetGrowdepaotUrl) => {
    try {
      const fetchData = await Axios.get(GetGrowdepaotUrl);
      setGrowdepaot(fetchData.data.data);
      let GetOthersUrl = `${process.env.REACT_APP_API_URI}growDepot/usergrowDepot?userId=${fetchData.data.data.userId?._id}`;
  ...
  useEffect(() => {
    ...
    let GetGrowdepaotUrl = `${process.env.REACT_APP_API_URI}growDepot/${routeParams.id}?latlang=${data?.location?.coordinates[0]},${data?.location?.coordinates[1]}`;
    GetGrowdepaots(GetGrowdepaotUrl);
  ...
  const images = [];
  if (growDepot?.photo) {
    if (Array.isArray(growDepot.photo)) {
      growDepot.photo.forEach((data) => {
        images.push({
      ...
      images.push({
        original: `${process.env.REACT_APP_PORT}/${growDepot.photo}`,
        thumbnail: `${process.env.REACT_APP_PORT}/${growDepot.photo}`,
      });
    }
  }
    ...
    <div className="product-user-profile">
      <div className="container mx-auto">
        <div className="d-sm-flex d-none align-items-center gap-2 font-18-100 font-weight-500 mb-4 ps-12">
          ...
          &gt;
          <span className="text-grey">{growDepot.name}</span>
        </div>
            ...
            <div className="ps-sm-0 ps-3">
              <div className="border-smx-bottom mb-4">
                ...
                <div className="d-flex gap-sm-5 gap-3 align-items-sm-center gap-2 mb-sm-4 mb-3 flex-sm-row flex-column">
                  <div>
                    <span className="d-flex gap-2 align-items-center font-18 mb-sm-4 mb-3 font-weight-500">
                      ...
                      <span>{growDepot?.userId?.storeName}</span>
                    </span>
                    <span className="d-flex gap-2 align-items-center font-18 font-weight-500">
                      <DistanceIcon />
                      <span>{growDepot.distance} Away </span>
                    </span>
                  </div>
                  <div>
                    <div className="d-flex gap-2 align-items-center flex-wrap mb-sm-4 mb-3">
                      <span className="d-flex gap-2 align-items-center font-24 font-weight-700">
                        <RatingIcon />
                        <span>{growDepot?.userId?.ratingsAverage}</span>
                      </span>
                      <span className="font-18-100 text-grey font-weight-400">
                        <span>
                          ({growDepot?.userId?.ratingsQuantity} Reviews)
                        </span>
                      </span>
                    </div>
                    <span className="d-flex gap-2 align-items-center font-18 font-weight-500">
                      <TimerIcon />
                      <span>{`Store Hours: ${growDepot?.userId?.startTime} To ${growDepot?.userId?.closeTime}`}</span>
                    </span>
                  </div>
                </div>
                ...
                <span className="d-flex gap-2 align-items-center font-18 font-weight-500 mb-sm-4 pb-sm-1 mb-3">
                  <LocationIcon />
                  <span>{growDepot.userId?.location?.address}</span>
                </span>
              </div>
              <p className="font-24 font-weight-700">
                {growDepot.userId?.fullName}
              </p>
              <hr />
              <p className="mt-3 font-18 font-weight-500">
                Description: {growDepot.description}
              </p>
              ...
              <div className="d-flex flex-sm-row flex-column justify-content-between align-items-center gap-sm-4 gap-3 mt-md-5 mt-3 pt-4">
                ...
                >
                  <span>
                    {growDepot.favourite &&
                    growDepot.favourite.includes(currentuserData._id)
                      ? "Mark Unfavourite"
                      : "Mark Favourite"}
                  </span>
                ...
                {growDepot?.userId?.location?.phone && (
                  <a
                    href={`tel:${growDepot?.userId?.location?.phone}`}
        ...
        <h3 className="d-flex gap-2 align-items-center flex-wrap font-32 font-weight-700 pt-3 mt-5 ms-12 bordered-heading">
          ...
          <span className="text-primary-green">
            {growDepot.userId?.fullName || growDepot.userId?.storeName}
          </span>
        </h3>
            ...
            <select
              ...
              onChange={(e) => {
                ...
                GetOthersByUser(
                  ...
                  }growDepot/usergrowDepot?quantity=${e.target.value}${
                    selectedStrain ? `&postStrain=${selectedStrain}` : ""
                  }&userId=${growDepot?.userId?._id}`
                );
              }}
            >
          ...
          <div className="col-lg-3 col-md-6  bg-transparent border-0">
            ...
            <select
              ...
              onChange={(e) => {
                ...
                GetOthersByUser(
                  `${process.env.REACT_APP_API_URI}growDepot/usergrowDepot?${
