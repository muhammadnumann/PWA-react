...
const DispensaryMap = () => {
  ...
  const clearFilterHandler = (e) => {
    ...
    const currentUser = localStorage.getItem("userdata");
    let data = JSON.parse(currentUser);
    let GetDispensaryUrl = `${process.env.REACT_APP_API_URI}users/getAllData/?category=dispensary&userType=retailer&latlang=${data?.location?.coordinates[0]},${data?.location?.coordinates[1]}`;
    GetDispensary(GetDispensaryUrl);
  };
  console.log(filter);
