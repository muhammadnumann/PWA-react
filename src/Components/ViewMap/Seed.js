...
const SeedMap = () => {
  ...
  const clearFilterHandler = (e) => {
    ...
    const currentUser = localStorage.getItem("userdata");
    let data = JSON.parse(currentUser);
    let GetSeedsUrl = `${process.env.REACT_APP_API_URI}users/getAllData/?userType=consumer&name=${searchTerm}&latlang=${data?.location?.coordinates[0]},${data?.location?.coordinates[1]}`;
    GetSeeds(GetSeedsUrl);
  };
      ...
      <div className="allproduct-mob d-block">
        <div className="container mx-auto mb-5">
          <div className="d-flex align-items-center justify-content-between gap-4">
            <h2 className="allproduct-heading">All Seed / Bud</h2>
            <button
