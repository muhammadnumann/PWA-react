...
const CannabisMap = () => {
  ...
  const clearFilterHandler = (e) => {
    ...
    const currentUser = localStorage.getItem("userdata");
    let data = JSON.parse(currentUser);
    let GetCannabisUrl = `${process.env.REACT_APP_API_URI}users/getAllData/?category=cannabisLaounge&userType=retailer&latlang=${data?.location?.coordinates[0]},${data?.location?.coordinates[1]}`;
    GetCannabis(GetCannabisUrl);
  };
      ...
      <div className="allproduct-mob d-block">
        <div className="container mx-auto mb-5 ">
          <div className="d-flex align-items-center justify-content-between gap-4">
            <h2 className="allproduct-heading">All Cannabis Lounge</h2>
            <button
