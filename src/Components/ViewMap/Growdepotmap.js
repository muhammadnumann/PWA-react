...
const Growdepotmap = () => {
  ...
  const clearFilterHandler = (e) => {
    ...
    const currentUser = localStorage.getItem("userdata");
    let data = JSON.parse(currentUser);
    let GetGrowDepotUrl = `${process.env.REACT_APP_API_URI}users/getAllData/?category=growDepot&userType=retailer&latlang=${data?.location?.coordinates[0]},${data?.location?.coordinates[1]}`;
    GetGrowdepot(GetGrowDepotUrl);
  };
  return (
    <div className="ps-12 pe-12">
      ...
      {Growdepot?.result?.length !== 0 ? (
        <div className="row flex-md-row flex-column-reverse ms-12 me-12 seed-card flex-row ps-12 pe-12">
          <div className="12 p-0 mb-md-0">
                  ...
                  >
                    <div className="d-flex justify-content-end w-100 poeple-sharing-seed">
                      <button className="green-btn-outline bg-white z-index-1 text-black d-flex align-items-center justify-content-between font-18 py-sm-3 gap-2 w-max-content">
                        ...
                        {Growdepot.length} People Sharing Seeds
                      </button>
                    </div>
                    <GoogleMapNew
                      markersData={Growdepot.result}
