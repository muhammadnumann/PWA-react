...
const Cannabis = (props) => {
  ...
  const submitHandler = (event) => {
    ...
    }&category=cannabisLounge`;
    GetCannabis(GetAllProductUrl);
  };
  ...
  const GetCannabis = async (GetCannabisUrl) => {
    try {
      const fetchData = await DashboardAxios.get(GetCannabisUrl);
  ...
  const GetCannabisUseEffect = async (GetCannabisUrl) => {
    try {
  ...
  useEffect(() => {
    ...
    GetCannabisUseEffect(GetCannabisUrl);
  }, []);
  ...
  const clearFilterHandler = (e) => {
    ...
    const currentUser = localStorage.getItem("userdata");
    let data = JSON.parse(currentUser);
    let GetCannabisUrl = `${process.env.REACT_APP_API_URI}users/getAllData/?page=${page}&category=cannabisLaounge&userType=retailer&name=${searchTerm}&latlang=${data?.location?.coordinates[0]},${data?.location?.coordinates[1]}`;
    GetCannabis(GetCannabisUrl);
  };
  return (
    <>
        ...
        <div className="allproduct-mob d-block">
          <div className="container mx-auto">
            <div className="d-flex align-items-center justify-content-between gap-4 ps-12 pe-12">
              <h2 className="allproduct-heading ms-12 me-12">
                All Cannabis Lounge
              </h2>
              <button
