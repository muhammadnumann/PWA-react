...
const Dispensary = (props) => {
  ...
  const GetDispensary = async (GetDispensaryUrl) => {
    try {
      const fetchData = await DashboardAxios.get(GetDispensaryUrl);
  ...
  const GetDispensaryUseEffect = async (GetDispensaryUrl) => {
    try {
  ...
  useEffect(() => {
    ...
    GetDispensaryUseEffect(GetDispensaryUrl);
  }, []);
  ...
  const submitHandler = (event) => {
    ...
    GetDispensary(GetAllProductUrl);
  };
  ...
  const clearFilterHandler = (e) => {
    ...
    const currentUser = localStorage.getItem("userdata");
    let data = JSON.parse(currentUser);
    let GetDispensaryUrl = `${process.env.REACT_APP_API_URI}users/getAllData/?page=${page}&category=dispensary&userType=retailer&name=${searchTerm}&latlang=${data?.location?.coordinates[0]},${data?.location?.coordinates[1]}`;
    GetDispensary(GetDispensaryUrl);
  };
  return (
    <>
                ...
                <div className="d-flex flex-column align-items-start justify-content-center mb-5 mt-4 pt-2">
                  ...
                  <div className="form-control h-auto p-0 bg-transparent border-0">
                    ...
                    >
                      ...
                      <option value={"1-7"}>1-4 Grams</option>
                      <option value={"7-14"}>5-10 Grams</option>
                      <option value={"14-30"}>11-15 Grams</option>
                    </select>
                  </div>
                </div>
